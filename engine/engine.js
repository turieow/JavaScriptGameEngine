...
class EventDispatcher {
	...
	dispatchEvent(type, event) {
		...
		if(listeners != undefined) listeners.forEach(callback => {
			callback(event);
		});
	}
}
...
class Actor extends EventDispatcher {
	...
	set y(value){
		this._y = value;
		this._hitAreaOffsetY = value + this._hitAreaOffsetY;
	}
}
...
class Scene extends EventDispatcher {
	constructor (name, backgroudColor, renderingTarget) {
		super();
...
class Game {
	...
	changeScene(newScene) {
		...
		console.log('sceneが${newScene.name}に切り替わりました。');
	}
